...
const { selector, keccak256 } = require('./keccak');
 ...
 *
 * Written here rather than imported so that a decode that goes wrong is a
 * fault in ~200 readable lines, and so both stacks carry the same coder and
 * can be checked against each other on the same bytes. Verified in
 * abi.test.js against vectors produced by `cast`.
 *
...
/** keccak topic of an event signature, 0x-prefixed. */
function topic(signature) {
  return '0x' + keccak256(Buffer.from(signature, 'utf8')).toString('hex');
}
...
module.exports = { parseType, isDynamic, encodeCall, encodeTuple, encodeValue, decode, named, topic, decodeError, errorTable, word };
